#Este módulo contiene las funciones originalmente en el script complejosSimpliciales1_notas_acordes_intervalos_ritmos_sin_octavas_sin_inversiones.py
...
from plot_barcodes_ripser import plot_barcodes #plot_barcodes_ripser es un módulo artesanal para graficar los códigos de barras a partir de la información de los diagramas de persistencia generados por ripser


#lo siguiente contempla extraer de un archivo de notacion musical lo siguiente: notas_acordes_intervalos_ritmos_sin_octavas_sin_inversiones
def get_data_TDA(path,inicio=1,fin=4,todo=False,showScore=False,showChords=False):#inicio,fin: compases de inicio, fin del score
    data_TDA={}
    i_diag=path.rfind('/')
    titulo=path[i_diag+1:-4]
    score=m21.converter.parse(path)#NOTA: hay que quitar en el archivo .xml o .mxl, etc los pentagramas que sean de percusiones no afinadas
    eventos=[]
    eventosTDA=[]
    eventosB=[]
    if todo==True:
        fin=len(score.parts[0].getElementsByClass('Measure'))
    if showScore==True:
        score.measures(inicio,fin).show()
    #acs=score.chordify() #esta instruccion es para considerar TODO el archivo.
    titulo_frag="{}, cc.{}-{}".format(titulo,str(inicio),str(fin))
    acs=score.measures(inicio,fin).chordify()
    if showChords==True:
    ...
    data_TDA['titulo_frag']=titulo_frag
    data_TDA['eventos']=eventos
    data_TDA['eventosTDA']=eventosTDA
    data_TDA['eventosB']=eventosB
    return data_TDA
...
def TDA_acs_int_rit_pos(path,inicio=1,fin=4,todo='False',showTDAplots=True,showScore='False',showChords='False'):
    data_TDA=get_data_TDA(path,inicio,fin,todo,showScore,showChords)
    diccTDAdgms={}
    #TDA sobre los eventosTDA: (acorde, duración, offset)    NOTA: faltaría incluir timbre, dinámica...
    eventos=data_TDA['eventos']
    eventosTDA=data_TDA['eventosTDA']
    eventosB=data_TDA['eventosB']
    titulo_frag=data_TDA['titulo_frag']
    ...
    diccTDAdgms["dataeventosTDA"]=dgms
    l=[]
    Pers_entr=0
    D=0
    for i in dgms:
    ...
    for x in l:
        if x!=np.inf:
            ...
            continue
    print("entropía persistente de eventosTDA: ", Pers_entr)
    print("\n")



    if showTDAplots==True:
    ...
    if showTDAplots==True:
        ...
        plot_barcodes(dgms,titulo_frag,"Barcodes for $(vectInt(ac), dur , pos)$")
    ...
    print(dataeventosB)
    dgms = ripser(dataeventosB, maxdim=3, metric='euclidean')['dgms']
    diccTDAdgms["dataeventosB_VectInt"]=dgms
    for i in dgms:
        for j in i:
            print(j)
        print("\n")
    ...
    dgms = ripser(dataAcsR12_u, metric='euclidean')['dgms']
    diccTDAdgms["dataeventos_I12"]=dgms
    for i in dgms:
        for j in i:
            print(j)
        print("\n")
    if showTDAplots==True:
        plot_diagrams(dgms,title=titulo_frag+' '+'-'+' '+"Acordes en $I^{12}$", colormap='default',show=True)
        plot_diagrams(dgms,title=titulo_frag+' '+'-'+' '+"Acordes en $I^{12}$", colormap='default',lifetime=True, show=True)
        fig.tight_layout()

        plot_barcodes(dgms,titulo_frag,"Barcodes for chord in $I^{12}$")
    ...
    dgms = ripser(dataAcsR12, metric='euclidean')['dgms']
    diccTDAdgms["dataeventos_R12_Tam_Alt"]=dgms
    for i in dgms:
        for j in i:
            print(j)
        print("\n")
    if showTDAplots==True:
        plot_diagrams(dgms, title=titulo_frag+' '+'-'+' '+'Acordes en $\mathbb{R} ^{12} , c.a. \overline {12} - \overline {23}$', colormap='default',show=True)
        plot_diagrams(dgms, title=titulo_frag+' '+'-'+' '+'Acordes en $\mathbb{R} ^{12} , c.a. \overline {12} - \overline {23}$', colormap='default',lifetime=True, show=True)
    ...
    dgms = ripser(datavectAcsInt, metric='euclidean')['dgms']
    diccTDAdgms["dataeventos_R12_Int_Alt"]=dgms
